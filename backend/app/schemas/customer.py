...
class CustomerProfileResponse(BaseModel):
    ...
    purchase_trend: List[Any] = Field(
        default_factory=list,
    )

    revenue_distribution: List[Any] = Field(
        default_factory=list,
    )

    order_frequency: List[Any] = Field(
        default_factory=list,
    )

    spending_growth: List[Any] = Field(
        default_factory=list,
    )
...
class CustomerStatisticsResponse(BaseModel):
    ...
    vip_revenue: float = 0.0

    average_customer_spend: float = 0.0

    vip_average_spend: float = 0.0
